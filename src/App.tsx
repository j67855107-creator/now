...
`;
  ...
  const handleFileSelected = (selectedFile: File) => {
    ...
    // Strict 50MB frontend size limit check
    const MAX_SIZE_MB = 50;
    if (selectedFile.size > MAX_SIZE_MB * 1024 * 1024) {
      triggerAlert("error", `File size limits exceeded: Your file is ${(selectedFile.size / (1024 * 1024)).toFixed(1)}MB, which exceeds the maximum secure 50MB limit.`);
      return;
    }
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-slate-400 text-xs">
                        {file ? `${file.name} (${file.size > 1024 * 1024 ? `${(file.size / (1024 * 1024)).toFixed(1)} MB` : `${(file.size / 1024).toFixed(1)} KB`})` : `Supports .docx, .pdf up to 50MB`}
                      </p>
                    </div>
