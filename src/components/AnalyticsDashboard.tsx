...
export default function AnalyticsDashboard({ stats, loading, onRefresh, token }: AnalyticsDashboardProps) {
  ...
  const downloadStatsFile = async () => {
    ...
    try {
      ...
      const response = await fetch(API_BASE + "/api/admin/download?file=stats.json", { headers });
      if (response.ok) {
  ...
  const downloadSubmissionsFile = async () => {
    ...
    try {
      const headers2: Record<string, string> = activeToken ? { Authorization: "Bearer " + activeToken } : {};
      const response = await fetch(API_BASE + "/api/admin/download?file=contact_submissions.json", { headers: headers2 });
      if (response.ok) {
        const blob = await response.blob();
        const url = URL.createObjectURL(blob);
        const link = document.createElement("a");
  ...
  const extCounts = stats.recentLogs.reduce((acc: Record<string, number>, curr) => {
    acc[curr.fileExt] = (acc[curr.fileExt] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  const pdfCount = extCounts["pdf"] || 4;
    ...
    <div id="analytics-panel" className="space-y-8 select-none">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm lg:col-span-2 flex flex-col justify-between text-left">
          ...
          <div className="w-full h-[180px] flex items-end justify-around border-b border-slate-200 pb-2 relative">
            ...
            <div className="flex flex-col items-center gap-2 group w-16">
              ...
              <div className="w-8 bg-indigo-500 hover:bg-indigo-600 rounded-t-lg transition-all duration-300 shadow-sm"
            ...
            <div className="flex flex-col items-center gap-2 group w-16">
              ...
              <div className="w-8 bg-emerald-500 hover:bg-emerald-600 rounded-t-lg transition-all duration-300 shadow-sm"
            ...
            <div className="flex flex-col items-center gap-2 group w-16">
              ...
              <div className="w-8 bg-rose-500 hover:bg-rose-600 rounded-t-lg transition-all duration-300 shadow-sm"
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden text-left">
        <div className="px-6 py-4 border-b border-slate-150 flex flex-col sm:flex-row sm:items-center justify-between gap-3 select-none">
          <div className="text-left">
            <h3 className="font-bold text-slate-900 text-base tracking-tight font-sans">Recent Conversion Ledger</h3>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 border border-slate-200 p-1 rounded-lg bg-slate-50">
              ...
              <button onClick={() => setFilterMode("ai")}
              ...
              <button onClick={() => setFilterMode("classic")}
            ...
            <button onClick={downloadStatsFile} disabled={downloadingStats} id="btn-download-stats-json"
              ...
              {filteredLogs.length > 0 ? (
                filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-900 whitespace-nowrap overflow-hidden max-w-[200px] truncate" title={log.fileName}>{log.fileName}</td>
                    <td className="px-6 py-4 uppercase font-semibold text-xs text-slate-400">{log.fileExt}</td>
                    <td className="px-6 py-4 text-xs font-mono text-slate-500 table-cell">{log.fileSizeKb > 1024 ? `${(log.fileSizeKb / 1024).toFixed(1)} MB` : `${log.fileSizeKb} KB`}</td>
                    <td className="px-6 py-4">
                      {log.mode === "ai" ? (
                        <span className="flex items-center gap-1.5 text-xs text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full font-medium w-fit border border-amber-100">
                          <Zap size={10} className="fill-indigo-300" /> AI-Powered
                        </span>
                      ) : (
                        <span className="text-xs text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-full font-medium w-fit border border-indigo-100">Classic Core</span>
                      )}
                    </td>
                    <td className="px-6 py-4 font-mono text-xs font-semibold text-slate-800 table-cell">{log.durationMs}ms</td>
                    <td className="px-6 py-4 text-xs text-slate-400 table-cell" title={log.timestamp}>{formatTimeAgo(log.timestamp)}</td>
                    <td className="px-6 py-4">{log.status === "success" ? (
                      <span className="text-xs text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full font-semibold border border-emerald-100 flex items-center gap-1 w-fit"><CheckCircle size={12} /> Active</span>
                    ) : (
                      <span className="text-xs text-rose-700 bg-rose-50 px-2.5 py-1 rounded-full font-semibold border border-rose-100 flex items-center gap-1 w-fit">Failed</span>
                    )}</td>
                  </tr>
                ))
              ) : (
                <tr><td colSpan={7} className="text-center font-sans italic py-12 text-slate-400">No conversion activities found matching the selections.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden text-left">
        <div className="px-6 py-4 border-b border-slate-150 flex flex-col sm:flex-row sm:items-center justify-between gap-3 select-none">
          <div className="text-left">
            <h3 className="font-bold text-slate-900 text-base tracking-tight font-sans">Support Inbox & Contact Queries</h3>
          ...
          <button onClick={downloadSubmissionsFile} disabled={downloadingSubmissions} id="btn-download-contact-submissions-json"
              ...
              {stats.contactSubmissions && stats.contactSubmissions.length > 0 ? (
                stats.contactSubmissions.map((sub) => (
                  <tr key={sub.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 font-semibold text-slate-900 whitespace-nowrap">{sub.name}</td>
                    <td className="px-6 py-4 font-mono text-xs text-indigo-600"><a href={`mailto:${sub.email}`} className="hover:underline">{sub.email}</a></td>
                    <td className="px-6 py-4 text-xs font-semibold text-slate-800 whitespace-nowrap truncate max-w-[150px]" title={(sub as any).subject}>{(sub as any).subject || "(No Subject)"}</td>
                    <td className="px-6 py-4 text-xs text-slate-600 max-w-md truncate" title={sub.message}>{sub.message}</td>
                    <td className="px-6 py-4 text-xs text-slate-400">{formatTimeAgo(sub.timestamp)}</td>
                  </tr>
                ))
              ) : (
                <tr><td colSpan={5} className="text-center font-sans italic py-12 text-slate-400">No support tickets found. Messages will show up here after submission.</td></tr>
              )}
            </tbody>
          </table>
        </div>
