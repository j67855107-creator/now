...
// Trust front-facing proxies (Vercel, Railway, Cloud Run etc.) for accurate rate limiting
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", "https://*.run.app", "https://convertoneai.com", "https://*.railway.app"],
  ...
  frameguard: false // Disabled to allow rendering within the AI Studio development preview iframe
}));
...
const convertLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 50, // Slightly increased for power users, but still highly secure
  standardHeaders: true,
  legacyHeaders: false,
...
const allowedOrigins = [
  "https://convertoneai.com",
  "https://www.convertoneai.com",
  "http://localhost:5173",
  "http://localhost:3000"
];
...
app.use(cors({
  origin: (origin, callback) => {
    if (!origin) return callback(null, true); // Allow curl / non-browser requests
...
// Body parsing with strict 75mb limit to safely support up to 50MB files (base64 encoded)
...
// Markdown Sanitize Helper for XSS and Script Injection Protection
function sanitizeMarkdown(markdown: string): string {
  ...
  // Remove inline javascript handlers (like onload, onclick, etc.) from any HTML elements
  sanitized = sanitized.replace(/on\w+\s*=\s*(['"][^'"]*['"]|[^>\s]+)/gi, "");
  
  // Remove other highly sensitive HTML elements that can lead to XSS/defacement
  sanitized = sanitized.replace(/<(iframe|object|embed|link|meta|style|form|input|button|textarea)[^>]*>([\s\S]*?)<\/\1>/gi, "");
  sanitized = sanitized.replace(/<(iframe|object|embed|link|meta|style|form|input|button|textarea)[^>]*\/?>/gi, "");
  
  // Remove javascript: and data: URIs in markdown links/images to prevent standard prompt redirection XSS
  sanitized = sanitized.replace(/\[([^\]]+)\]\(\s*(javascript|data):([^\)]+)\)/gi, "[$1](#unsafe-link)");
...
app.post("/api/convert", convertLimiter, requireApiKey, async (req: Request, res: Response): Promise<void> => {
  ...
  let { fileData, fileName, mimeType } = req.body;
  ...
  // 3. Strict 50MB size limit to avoid Heap crashes or memory exhaust attacks
  const sizeBytes = Buffer.from(cleanedBase64, 'base64').length;
  const fileSizeKb = sizeBytes / 1024;
  const MAX_FILE_SIZE_KB = 50 * 1024; // 50MB limit

  if (fileSizeKb > MAX_FILE_SIZE_KB) {
    res.status(400).json({ 
      error: `File transmission blocked: Doc size (${(fileSizeKb / 1024).toFixed(1)}MB) exceeds the secure 50MB limit for volatile server processing.` 
    });
    return;
  }
  ...
  try {
    ...
    // Secure Markdown Sanitization (Defensive Security Engineering against XSS)
    let cleanMarkdown = sanitizeMarkdown(outputMarkdown.trim());

    const durationMs = Date.now() - startTimestamp;
    recordConversion(safeFileName || "unnamed_document", fileExt, fileSizeKb, actualMode, "success", durationMs);

    // Memory cleanups to optimize node heap space
    buffer = null;
    outputMarkdown = "";
    ...
    buffer = null; // Free buffer reference on error
